...
def run_nuts_numpyro(cfg, obs: dict, fm: Callable, exp_dir) -> Dict[str, jnp.ndarray]:
    ...
    # Initialize chains with small perturbations for diversity
    if chains > 1:
        rng_key, init_key = jax.random.split(rng_key)
        # Add small normal perturbations (std=0.1) to each chain's initial u
        perturbations = jax.random.normal(init_key, shape=(chains,) + init_u.shape) * 0.1
        init_u = init_u[None, :] + perturbations  # (chains, dim_free)
