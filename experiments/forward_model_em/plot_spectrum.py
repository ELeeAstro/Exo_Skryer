...
plot_spectrum.py
================
Plot an emission forward model spectrum saved by ``run_forward_model.py``.
...
-----
    ...
    python plot_spectrum.py --spectrum forward_spectrum_highres.txt --obs ../../obs_data/some_emission_obs.txt

    # Plot binned spectrum with obs overlay:
    python plot_spectrum.py --spectrum forward_spectrum_binned.txt --obs ../../obs_data/some_emission_obs.txt
"""
...
def plot_forward_spectrum(
    ...
    if obs_path is not None:
        ...
        obs_flux = obs_raw[:, 2].astype(float)
        obs_err = obs_raw[:, 3].astype(float)
        ax.errorbar(
            obs_wl, obs_flux, xerr=obs_dwl, yerr=obs_err,
    ...
    ax.set_ylabel(r"Flux [erg s$^{-1}$ cm$^{-2}$ cm$^{-1}$]", fontsize=14)
    ax.set_xscale("log")
...
def main():
    ap = argparse.ArgumentParser(description="Plot forward model emission spectrum")
